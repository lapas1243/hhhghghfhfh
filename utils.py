...
SOLANA_ADMIN_WALLET = os.environ.get("SOLANA_ADMIN_WALLET", "") # Admin wallet for receiving payments
...
LOGS_CHANNEL_ID_RAW = os.environ.get("LOGS_CHANNEL_ID", None)  # Channel ID for payment logs
...
# Logs Channel for payment transparency
LOGS_CHANNEL_ID = None
if LOGS_CHANNEL_ID_RAW is not None:
    try: LOGS_CHANNEL_ID = int(LOGS_CHANNEL_ID_RAW)
    except (ValueError, TypeError): logger.error(f"Invalid format for LOGS_CHANNEL_ID: {LOGS_CHANNEL_ID_RAW}. Must be an integer (channel ID).")
...
if not SOLANA_ADMIN_WALLET: logger.warning("WARNING: SOLANA_ADMIN_WALLET not configured. Auto-sweep will be disabled.")
...
logger.info(f"Solana webhook expected at: {WEBHOOK_URL}/webhook")
logger.info(f"Telegram webhook expected at: {WEBHOOK_URL}/telegram/{TOKEN}")
if LOGS_CHANNEL_ID:
    logger.info(f"Logs channel configured: {LOGS_CHANNEL_ID}")
else:
    logger.info("Logs channel not configured. Set LOGS_CHANNEL_ID to enable payment logging.")
...
# Removed NOWPayments API URL - now using Solana only
...
def init_db():
    ...
    try:
        with get_db_connection() as conn:
            ...
            # solana_wallets table for Solana payment processing
...
def _get_lang_data(context: ContextTypes.DEFAULT_TYPE) -> tuple[str, dict]:
    ...
    # Handle None context (e.g., from background jobs)
...
async def send_purchase_log(
    ...
    Args:
        ...
        user_id: Buyer's Telegram user ID
        username: Buyer's username or first name
        amount_paid: Amount paid in crypto
        currency: Currency code (e.g., "SOL")
        payment_id: Unique payment/order ID
        is_success: Whether the payment was successful
        payment_type: Type of payment ("purchase" or "refill")
        tx_signature: Solana transaction signature for Solscan link
        eur_amount: EUR equivalent of the payment
        basket_snapshot: List of purchased products (for purchases only)
    """
    if not LOGS_CHANNEL_ID:
        logger.debug("Logs channel not configured, skipping purchase log.")
        return None
    ...
    try:
        ...
        if basket_snapshot and payment_type == "purchase":
            # Count products by name and size
            product_counts = {}
        ...
        if result:
            logger.info(f"📋 Purchase log sent to channel for payment {payment_id}")
        else:
            logger.warning(f"Failed to send purchase log for payment {payment_id}")
        
        return result
        
    except Exception as e:
        logger.error(f"Error sending purchase log for payment {payment_id}: {e}", exc_info=True)
        return None
...
# Removed get_nowpayments_min_amount - no longer needed for Solana-only implementation
